...
import { Zap, FileText, CreditCard, ShieldCheck, CheckCircle2, Wallet, Heart, Receipt, Info } from "lucide-react";
      ...
      <div className="container mx-auto px-4">
        <div className="max-w-6xl mx-auto space-y-20">
          <div className="text-center space-y-6">
            <Badge variant="outline" className="border-primary/30 text-primary bg-primary/5 px-4 py-1 uppercase text-[10px] font-bold tracking-widest">Billing & Checkout</Badge>
          ...
          <div className="grid lg:grid-cols-3 gap-8">
            {paymentMethods.map((method) => (
              <div key={method.name} className="relative group">
                ...
                <Card className="bg-card/40 backdrop-blur-3xl border-white/5 relative rounded-[2rem] overflow-hidden group-hover:bg-white/[0.03] transition-colors h-full">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={adv} className="flex items-center gap-3 group">
                      ...
                      <span className="text-xs font-bold text-muted-foreground/80 uppercase tracking-tighter">{adv}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="p-8 rounded-[2.5rem] bg-black/40 border border-white/10 shadow-inner space-y-8">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase text-muted-foreground">Gateway Provider</p>
                      <p className="text-sm font-bold">Asaas Instituição de Pagamento</p>
                    </div>
                  </div>
                  <Badge className="bg-emerald-500/20 text-emerald-400 border-none px-3 py-1 font-bold text-[10px]">API STABLE</Badge>
                </div>
                  ...
                  <div className="space-y-3">
                    <Button asChild className="w-full h-14 rounded-2xl bg-accent text-accent-foreground hover:scale-[1.02] transition-all font-bold text-md shadow-lg shadow-accent/20">
                      <Link href="https://www.asaas.com/c/2h5zwl883x3g3erc" target="_blank">
                        <Heart className="w-5 h-5 mr-2 fill-current" /> Apoiar ou Contratar
                      </Link>
                    </Button>
                    <p className="text-center text-[10px] text-muted-foreground/60 uppercase font-bold tracking-[0.2em]">
                      Link verificado • Ambiente 100% seguro
                    </p>
                  </div>
                </div>
